chcp 65001
ECHO OFF
ECHO Код Хэмминга. Декодировщик.
ECHO Текстовый файл должен содержать двоичный код
ECHO Автоматически исправляет 1 ошибку в коде
...
ECHO 1. Двоичный код, который нужно декодировать.
set /P in="> ":
ECHO 2. Путь для сохранения декодированного кода
set /P encoded="> ":
